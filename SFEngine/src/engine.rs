...
// ──────────────────────────────────────────
// Finalize build — auto-detect project type and compile
// ──────────────────────────────────────────

async fn finalize_build(workspace: &str, mission_id: &str, on_event: &EventCallback) {
    use std::path::Path;
...
fn detect_build_artifacts(workspace: &str) -> Vec<String> {
...
async fn run_loop(
    ...
    for iteration in 1..=MAX_LOOP_ITERATIONS {
        ...
        let gate = check_gate_raw(&review_output);
        if gate != "vetoed" {
